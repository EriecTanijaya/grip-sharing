import { User } from ".";
import { IdnGreeter } from "./greeter/IdnGreeter";
...
describe("User", () => {
  ...
  test("should able to greet by male gender", () => {
    const greeting = user.greet({ name: "Steven", gender: "male" });

    expect(greeting).toContain("Mr");
  });

  test("should able to greet by female gender", () => {
    const greeting = user.greet({ name: "Steven", gender: "female" });
